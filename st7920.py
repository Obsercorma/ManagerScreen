...
class ST7920:
	def __init__(self):
		...
		self.spi.cshigh = False # use inverted CS
		self.spi.max_speed_hz = 530000 # set SPI clock to 1.8MHz, up from 125kHz
	...
	def put_text(self, s, x, y, invert_px=False):
		for c in s:
			try:
				...
				for row in char:
					sx = 0
					for px in row:
						self.plot(x+sx, y+sy, px != 1 if invert_px else px==1)
						sx += 1
					sy += 1
			except KeyError:
				pass
